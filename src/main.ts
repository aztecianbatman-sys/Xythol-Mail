...
function authBox(title:string,text:string){
  return '<div class="auth"><div class="authbox"><img src="/assets/branding/logo-mark.svg"><span class="eyebrow">XYTHOL MAIL</span><h1>'+esc(title)+'</h1><p>'+esc(text)+'</p></div></div>';
}
function showIdentity(){
  root.innerHTML='<div class="auth"><div class="authbox"><img src="/assets/branding/logo-mark.svg"><span class="eyebrow">ANONYMOUS IDENTITY</span><h1>Pick your Xythol name.</h1><p>No real email or password is required for your Xythol identity.</p><label>Name<input id="name" maxlength="32" placeholder="Night Owl"></label><div id="preview"></div><button class="primary full" id="create">Create identity</button><small class="hint">The Xythol address is an identity label, not a public internet mailbox.</small></div></div>';
...
function saveDraft(){const d={to:val('to'),subject:val('subject'),body:document.querySelector<HTMLTextAreaElement>('#body')?.value||'',updated:Date.now()};const all=JSON.parse(localStorage.getItem('xythol-drafts')||'[]');localStorage.setItem('xythol-drafts',JSON.stringify([d,...all].slice(0,50)));state.status='Draft saved locally';render();view='compose';render()}
async function sendMail(){const to=val('to'),subject=val('subject'),body=document.querySelector<HTMLTextAreaElement>('#body')?.value||'';if(!/^\S+@\S+\.\S+$/.test(to)){state.status='Add a valid recipient';render();return}const smtp={host:val('host'),port:Number(val('port')||587),username:val('user'),password:val('pass'),from:val('from'),security:(document.querySelector<HTMLSelectElement>('#security')?.value||'starttls')};if(!smtp.host||!smtp.username||!smtp.password||!smtp.from){state.status='Fill SMTP settings';render();return}state.status='Sending…';render();try{await invoke('send_email',{smtp,message:{to:[to],subject,body}});state.status='Sent';alert('Email accepted by SMTP')}catch(err){state.status='Send failed';alert(String(err))}render()}
boot();